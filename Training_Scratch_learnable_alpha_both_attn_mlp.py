...
class Block(nn.Module):
    ...
    def forward(self, x):
        # Pre-LN attention
        x = x + self.scale_1 * self.attn(self.ln_1(x))

        # Pre-LN MLP
        x = x + self.scale_2 * self.mlp(self.ln_2(x))
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        if losses['val'] < best_val_loss:
            ...
            ckpt_path = os.path.join(out_dir, "ckpt_best_bs_512_learnable_alpha_both_attn_mlp.pt")
            ckpt = {
                'model': raw_model.state_dict(),
                'optimizer': optimizer.state_dict(),
                'model_args': model_args,
